...
class OCRService:
    ...
    @staticmethod
    def _parse_table(file_bytes: bytes, is_csv = False) -> str:
        ...
        try:
            match content_type:
                case "application/pdf":
                    return await run_in_threadpool(OCRService._parse_pdf, file_bytes)
                case "image/png" | "image/webp" | "image/jpeg":
                    return await run_in_threadpool(OCRService._parse_image, file_bytes)
                case "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" | "application/vnd.ms-excel":
                    return await run_in_threadpool(OCRService._parse_table, file_bytes, is_csv = False)
                case "text/csv":
                    return await run_in_threadpool(OCRService._parse_table, file_bytes, is_csv = True)
                case _:
